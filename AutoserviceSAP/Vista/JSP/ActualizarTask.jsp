...
<body onload="" style="text-align: center;">
	...
	<div>
		...
		<img style="-webkit-user-select: none;" border="0"
			...
			height="74">
				...
				<form id="getProcess" action="Task" method="get">
					...
					<table id="proceso" style="margin: 0 auto;" border="0" class="">
						<tr bgcolor="A2B8D2">
							<td width="250px" align="right"><b>Folio:</b></td>
							<td align="left"><input 
								type="text"
						...
						<tr bgcolor="A2B8D2">
							<td width="250px" align="right"><b>Clave:</b></td>
							<td><input 
							type="text" 
							name="cve" 
							id="cve"
					...
					<table id="operarTarea" style="margin: 0 auto;" border="0"
						...
						<c:if test="${not empty taskList}">
							...
							<c:forEach var="tarea" items="${taskList}">
								<tr bgcolor="A2B8D2">
									...
									<td><input type="text" name="taskId"
										value="${tarea.taskId}" readonly></td>
									<td><input type="text"
										name="processInstanceUuId_${tarea.taskId}"
										value="${tarea.processInstanceUuId}" readonly></td>
									<td><input type="text" name="name_${tarea.taskId}"
										value="${tarea.name}" readonly></td>
									<td><input type="text" name="actualOwner_${tarea.taskId}"
										value="${tarea.actualOwner != null && !tarea.actualOwner.isEmpty() ? tarea.actualOwner : 'sin asignar'}">
									</td>
									<td><input type="text"
										name="completionTime_${tarea.taskId}"
										value="${tarea.completionTime}" readonly></td>
									<td class="hidden"><input type="text" name="cve"
										value="${empleado.cve_empleado}"></td>
									<td class="hidden"><input type="text"
										name="processInstanceUuId"
										value="${tarea.processInstanceUuId}"></td>
									<td><select name="taskStatus_${tarea.taskId}">
											<option value="2" ${tarea.taskStatus == 2 ? 'selected' : ''}>Lista</option>
											<option value="3" ${tarea.taskStatus == 3 ? 'selected' : ''}>Reservada</option>
											<option value="4" ${tarea.taskStatus == 4 ? 'selected' : ''}>En
												Curso</option>
											<option value="12"
												${tarea.taskStatus == 12 ? 'selected' : ''}>Obsoleta</option>
											<option value="13"
												${tarea.taskStatus == 13 ? 'selected' : ''}>Cerrada</option>
									</select></td>
								</tr>
							</c:forEach>
						</c:if>
						...
						<tr>
							<td colspan="3" align="right">
								...
								<button type="submit" class="ui-state-active"
									style="margin-top: 20px">Actualizar Tareas</button>
							</td>
							<td colspan="3" align="left">
								<button type="button" id="ocultar" class="ui-state-active"
									style="margin-top: 20px"
									onClick="location.href='ActualizarTask.jsp';">Otra
									Tarea</button>
							</td>
						</tr>
					</table>
